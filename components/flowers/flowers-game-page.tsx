...
const flowersApiBasePath = flowersGame.apiBasePath;
...
const flowersSessionLabel = `${flowersGame.sessionLabel[0].toUpperCase()}${flowersGame.sessionLabel.slice(1)}`;
...
export function FlowersGamePage() {
  ...
  const [partyName, setPartyName] = useState(flowersDefaultPartyName);
  ...
  async function refreshSavedParties() {
    const response = await fetch(flowersApiBasePath, {
      method: "GET",
      cache: "no-store",
    });
    ...
    if (!response.ok) {
      throw new Error(
        await getApiErrorMessage(
          response,
          `Impossible de charger les ${flowersGame.sessionLabelPlural} sauvegardees.`,
        ),
      );
    }
  ...
  function resetParty(showToast = true) {
    ...
    setPartyName(flowersDefaultPartyName);
  ...
  persistPartyRef.current = async (saveMode: SaveMode) => {
    ...
    try {
      ...
      if (saveMode === "manual") {
        pushToast(
          `${flowersSessionLabel} "${payload.party.name}" sauvegardee.`,
          "success",
        );
      }
    } catch (error) {
  ...
  async function handleLoadParty(nextPartyId: string) {
    ...
    try {
      ...
      if (targetParty && !targetParty.isActive) {
        const restoreResponse = await fetch(`${flowersApiBasePath}/${nextPartyId}`, {
          method: "PATCH",
          headers: {
            "Content-Type": "application/json",
          },
      ...
      const response = await fetch(`${flowersApiBasePath}/${nextPartyId}`, {
        method: "GET",
        cache: "no-store",
      });
      ...
      pushToast(
        targetParty && !targetParty.isActive
          ? `${flowersSessionLabel} "${payload.party.name}" restauree et chargee.`
          : `${flowersSessionLabel} "${payload.party.name}" chargee.`,
        "info",
      );
    } catch (error) {
      pushToast(
  ...
  async function handleTogglePartyActive(targetPartyId: string, isActive: boolean) {
    ...
    try {
      ...
      const response = await fetch(`${flowersApiBasePath}/${targetPartyId}`, {
        method: "PATCH",
        headers: {
          "Content-Type": "application/json",
        },
      ...
      pushToast(
        isActive
          ? `${flowersGame.name} "${targetParty?.name ?? flowersGame.name}" restauree.`
          : partyId === targetPartyId
            ? `${flowersGame.name} "${targetParty?.name ?? flowersGame.name}" archivee et fermee.`
            : `${flowersGame.name} "${targetParty?.name ?? flowersGame.name}" archivee.`,
        "success",
      );
    } catch (error) {
      pushToast(
  ...
  async function handleRenameParty(targetPartyId: string, nextName: string) {
    ...
    try {
      ...
      await refreshSavedParties();
      pushToast(
        `${flowersSessionLabel} renommee en "${normalizedName}".`,
        "success",
      );
    } catch (error) {
      pushToast(
  ...
  async function handleDeleteParty(targetPartyId: string) {
    ...
    try {
      ...
      await refreshSavedParties();
      pushToast(
        `${flowersGame.name} "${deletedParty?.name ?? flowersGame.name}" supprimee.`,
        "success",
      );
    } catch (error) {
      pushToast(
  ...
  function handleAddPlayer(name: string) {
    ...
    if (players.length >= flowersMaxPlayers) {
      pushToast(
        `Une partie ${flowersGame.name} est limitee a ${flowersMaxPlayers} joueurs.`,
        "error",
      );
      return;
    }
  ...
  const isPlayerLimitReached = players.length >= flowersMaxPlayers;
